 ...
 *
 * Centralized location for all AI generation system prompts
 * Used across the application for consistent AI behavior
 */

/**
 * General AI-Assisted Text Generation System Prompt
 *
 * Used by: AIAssistedTextInput component via generateAIText server action
 * Purpose: Generate descriptions, summaries, and other text with strict anti-hallucination guardrails
 *
 * Key Features:
 * - Prevents hallucination and invention of details
 * - Scales output detail based on input detail
 * - Uses real examples from this AI chatbot project
 */
 ...
 *
 * Used by: generateTitleFromUserMessage in actions.ts
 * Purpose: Generate concise chat titles from first user message
 *
 * Key Features:
 * - Short, descriptive titles (max 80 chars)
 * - No quotes or colons
 * - Summarizes user intent
 */
...
/**
 * Knowledge Document Metadata Generation System Prompt
 *
 * Used by: generateDocumentMetadata in lib/ai/generate-document-metadata.ts
 * Purpose: Generate structured titles and classify uploaded knowledge documents
 *
 * Format: [date] - [type] - [purpose]
 *
 * Key Features:
 * - Structured title format for consistency
 * - Date extraction with "undated" fallback (no invention)
...
/**
 * Objective Document Title Generation System Prompt
 *
 * Used by: initializeVersionForChat in lib/db/objective-document.ts
 * Purpose: Generate document titles based on objective context
 *
 * Key Features:
 * - Reflects objective and document type
