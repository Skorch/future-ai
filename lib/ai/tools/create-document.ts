    ...
    execute: async ({ title, kind, documentType, fileUrl, content, metadata }) => {
      ...
      if (!processedContent && fileUrl) {
        try {
          ...
          throw new Error(`Could not retrieve file from ${fileUrl}: ${(error as Error).message}`);
        }
      }
      ...
      if (documentType === 'meeting-summary' && processedContent) {
        console.log('[CreateDocument] Meeting summary requested with transcript', {
      ...
      const responseMessage = documentType === 'meeting-summary' 
