...
import { generateObjectiveContext } from '@/lib/objective/objective-context';

const logger = getLogger('UpdateObjectiveContext');

interface UpdateObjectiveContextParams {
...
/**
 * AI tool for updating objective-specific context
 * AGGRESSIVE capture - focus on THIS specific goal/deal/project
 */
export const updateObjectiveContext = ({
  session,
  objectiveId,
  workspaceId,
}: UpdateObjectiveContextParams) =>
  tool({
    description: `Update objective context with facts about THIS SPECIFIC goal, deal, or project.
...
OBJECTIVE CONTEXT (aggressive) vs WORKSPACE CONTEXT (conservative):
...
❌ "Our standard approval process requires VP sign-off" (workspace context)`,
    ...
    execute: async ({ observations, updateReason }) => {
      logger.debug('Tool called: updateObjectiveContext', {
        objectiveId,
        workspaceId,
        updateReason,
      ...
      const result = await generateObjectiveContext({
        objectiveId,
        workspaceId,
