...
export function ObjectiveActionsTab({
  objectiveId,
  initialActions,
  actionsUpdatedAt: initialActionsUpdatedAt,
  placeholder,
  customLabels,
  ...
  const saveActions = useCallback(
    async (content: string) => {
      ...
      try {
        await updateObjectiveActionsAction(objectiveId, content);
        ...
        toast.error(
          error instanceof Error ? error.message : 'Failed to save actions',
        );
      } finally {
        setIsSaving(false);
      }
  ...
  const debouncedSave = useMemo(
    () => debounce((content: string) => saveActions(content), 2000),
    [saveActions],
  );
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        ...
        'aria-label': 'Objective actions editor',
      },
