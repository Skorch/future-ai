...
export async function POST(
  ...
  try {
    ...
    // Load full objective object (needed for builder)
    let objectiveObject = null;
    if (chatObjectiveId) {
      objectiveObject = await getObjectiveById(chatObjectiveId, userId);
    }

    // Create prepareStep function with full objects for builder
    ...
    logger.debug(
      `Current mode: ${currentMode}, Goal: ${modeContext.goal ? 'Set' : 'Not set'}, Todos: ${todos.length}`,
    );
    ...
    // NOTE: System prompt now generated by builder in prepareStep
    // We still need to calculate token usage for logging, but prepareStep provides the actual prompt
        ...
        try {
          ...
          try {
            ...
            const streamOptions: Parameters<typeof streamText>[0] = {
              ...
              // NO initial system prompt - prepareStep provides complete prompt for all steps
              // (prepareStep is called before Step 0, so it provides the initial prompt)
